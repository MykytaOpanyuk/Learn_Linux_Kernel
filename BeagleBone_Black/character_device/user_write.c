...
int usage(char **argv)
{
	printf("--> Program sends buffer to the specific device\n");
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < MEM_SIZE * 3; i++) {
		*timetable = (i * 13 + 8) % 256;
		timetable++;
	}

	result = write(fd_str.fd, buf, (MEM_SIZE * 3)); /* result = size of buf */

	if (result == 0)
		printf("ERROR - write dummy dev %d\n", result);
