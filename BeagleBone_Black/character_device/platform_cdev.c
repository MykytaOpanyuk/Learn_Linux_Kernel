                ...
                loff_t *f_pos)
{
	...
	if (cdevice->my_device && cdevice->my_device->dummy_read)
		return cdevice->my_device->dummy_read(cdevice->my_device, buf, count);

	return count;
}

ssize_t dummy_cdev_write(struct file *filp, const char __user *buf, size_t count,
		loff_t *f_pos)
{
	struct my_dummy_cdev *cdevice = filp->private_data;
	...
	if (cdevice->my_device && cdevice->my_device->dummy_write)
		return cdevice->my_device->dummy_write(cdevice->my_device, buf, count);

	return count;
}

static int dummy_cdev_mmap(struct file *filp, struct vm_area_struct *vma)
{
	struct my_dummy_cdev *cdevice = filp->private_data;

	if (cdevice->my_device && cdevice->my_device->dummy_mmap_read)
		return cdevice->my_device->dummy_mmap_read(cdevice->my_device, vma);

	if (cdevice->my_device && cdevice->my_device->dummy_mmap_write)
		return cdevice->my_device->dummy_mmap_write(cdevice->my_device, vma);
...
long dummy_cdev_ioctl(struct file *filp,
				 unsigned int cmd, unsigned long arg)
{
	...
	case DUMMY_SET_POOLING:
		if (cdevice->my_device && cdevice->my_device->set_poll_interval) {
			err = __get_user(interval, (u32 __user *)arg);
			if (err)
				break;

			err = cdevice->my_device->set_poll_interval(cdevice->my_device, interval);
		} else {
			err = -EINVAL;
		}
		break;
...
static int __init dummy_cdev_init(void)
{
	...
	for (i = 0; i < DUMMY_DEVICES; i++) {
		...
		dummy_cdevs[i].my_device = get_dummy_platfrom_device(i);
	...
	device_create(dummy_class, NULL, MKDEV(dummy_major, 0), NULL, "dummy" "%d", minor++);
	device_create(dummy_class, NULL, MKDEV(dummy_major, 1), NULL, "dummy" "%d", minor);
